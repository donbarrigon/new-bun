...
function executeCommand(command, description) {
  try {
    ...
    console.log(
      `${colors.green}✓ Comando ejecutado exitosamente${colors.reset}\n`
    );
  } catch (error) {
...
function updatePackageJson(projectName) {
  try {
    ...
    console.error(
      `${colors.red}✗ Error actualizando package.json: ${error}${colors.reset}`
    );
    process.exit(1);
  }
}
...
async function getProjectName() {
  while (true) {
    ...
    if (!projectName) {
      console.log(
        `${colors.red}✗ El nombre del proyecto no puede estar vacío${colors.reset}`
      );
      continue;
    }
...
async function initCommand() {
  console.log(
    `${colors.bold}${colors.magenta}🚀 Inicializando nuevo proyecto${colors.reset}\n`
  );
  ...
  console.log(
    `${colors.bold}${colors.green}🎉 Proyecto inicializado exitosamente!${colors.reset}`
  );
}

// Comando fork
async function initForkCommand() {
  console.log(
    `${colors.bold}${colors.magenta}🍴 Configurando fork del proyecto${colors.reset}\n`
  );
  ...
  console.log(
    `${colors.bold}${colors.green}🎉 Fork configurado exitosamente!${colors.reset}`
  );
}

// Comando update
function mergeUpstreamCommand() {
  console.log(
    `${colors.bold}${colors.magenta}🔄 Actualizando desde upstream${colors.reset}\n`
  );

  executeCommand("git fetch upstream", "📥 Obteniendo cambios del upstream");
  executeCommand(
    "git merge upstream/main",
    "🔀 Fusionando cambios del upstream"
  );

  console.log(
    `${colors.bold}${colors.green}🎉 Actualización completada!${colors.reset}`
  );
}
...
function showHelp() {
  console.log(
    `${colors.bold}${colors.cyan}Helper CLI - Asistente para proyectos${colors.reset}\n`
  );
  ...
  console.log(
    `  ${colors.green}init${colors.reset}           Inicializar un nuevo proyecto desde el kit de inicio`
  );
  console.log(
    `  ${colors.green}init:fork${colors.reset}      Configurar un fork del proyecto actual`
  );
  console.log(
    `  ${colors.green}merge:upstream${colors.reset} Actualizar desde el repositorio upstream`
  );
  console.log(
  ...
  try {
    switch (command) {
      ...
      case "merge:upstream":
        ...
        break;
      case "dev":
        executeCommand("bun .helper/build.js dev");
        break;
      case "build":
        executeCommand("bun .helper/build.js build");
        break;
      default:
        console.error(
